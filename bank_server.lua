...
-- The complete server implementation is kept here so this file can be
-- installed and launched by itself.
...
local function accountPath(id)
    return ACCOUNTS .. "/" .. id .. ".dat"
end

local function cardPath(id)
    return CARDS .. "/" .. id .. ".dat"
end

local function terminalPath(id)
    return TERMINALS .. "/" .. id .. ".dat"
end

local function logPath(id)
    return LOGS .. "/" .. id .. ".log"
end
...
local function account(id)
    return type(id) == "string" and load(accountPath(id)) or nil
end

local function card(id)
    return type(id) == "string" and load(cardPath(id)) or nil
end

local function terminal(id)
    return type(id) == "string" and load(terminalPath(id)) or nil
end
...
local function authenticateCard(cardId, pin)
    ...
    if tostring(c.pin) ~= tostring(pin or "") then
        return nil, nil, "INVALID_PIN"
    end
...
local function createAccount(name, balance, wantedPin)
    ...
    local c = {
        ...
        account_id = aid,
        owner_name = name,
        pin = tostring(wantedPin),
        status = "active",
        created_at = t
    }

    if not save(accountPath(aid), a) then
        return nil, "ACCOUNT_FILE_ERROR"
    end

    if not save(cardPath(cid), c) then
        fs.delete(accountPath(aid))
        return nil, "CARD_FILE_ERROR"
    end
...
local function changeBalance(aid, amount, mode, description)
    ...
    if mode == "withdraw" then
        if a.balance < amount then return nil, "INSUFFICIENT_FUNDS" end
    ...
    a.updated_at = now()
    if not save(accountPath(a.id), a) then
        return nil, "ACCOUNT_FILE_ERROR"
    end
    ...
    appendTx(a.id, {
        ...
        account_id = a.id,
        type = mode,
        amount = mode == "withdraw" and -amount or amount,
        balance_after = a.balance,
        currency = a.currency,
        description = description or mode,
        timestamp = now()
    })
...
local function registerTerminal(ownerCardId, ownerPin, name, existingId)
    ...
    if existingId and existingId ~= "" then
        ...
        if old.owner_account_id ~= a.id then
            return nil, "TERMINAL_OWNER_ONLY"
        end
        id = old.id
    else
    ...
    local t = {
        id = id,
        name = name and name ~= "" and tostring(name) or "Store Terminal",
    ...
    if not save(terminalPath(id), t) then
...
local function payment(data)
    ...
    if not save(accountPath(from.id), from) then
        return nil, "SOURCE_SAVE_ERROR"
    end
...
local function process(m)
    ...
    if m.action == "create_account" then
        local r, e = createAccount(d.owner_name, d.initial_balance, d.pin)
        return response(id, r ~= nil, r, e)
    end

    if m.action == "account_lookup" then
        local a = account(d.account_id)
        return response(id, a ~= nil, safeAccount(a), a and nil or "ACCOUNT_NOT_FOUND")
    end

    if m.action == "card_info" then
    ...
    if m.action == "balance" then
        local c, a, e = authenticateCard(d.card_id, d.pin)
        if not c then return response(id, false, nil, e) end
        return response(id, true, safeAccount(a))
    end

    if m.action == "transactions" then
        local r, e = getTransactions(d.card_id, d.pin, d.limit)
        return response(id, r ~= nil, r, e)
    end

    if m.action == "deposit" then
        local r, e = changeBalance(d.account_id, d.amount, "deposit", d.description)
        return response(id, r ~= nil, r, e)
    end

    if m.action == "withdraw" then
        local r, e = changeBalance(d.account_id, d.amount, "withdraw", d.description)
        return response(id, r ~= nil, r, e)
    end

    if m.action == "set_card_status" then
        local c = card(d.card_id)
        if not c then return response(id, false, nil, "CARD_NOT_FOUND") end
        if d.status ~= "active" and d.status ~= "blocked" then
            return response(id, false, nil, "INVALID_STATUS")
        end
        c.status = d.status
        if not save(cardPath(c.id), c) then
            return response(id, false, nil, "CARD_FILE_ERROR")
        end
        return response(id, true, {
            card_id = c.id,
            status = c.status
    ...
    if m.action == "register_terminal" then
        local r, e = registerTerminal(d.card_id, d.pin, d.terminal_name)
        return response(id, r ~= nil, r, e)
    end

    if m.action == "re_register_terminal" then
        local r, e = registerTerminal(d.owner_card_id, d.owner_pin, d.terminal_name, d.terminal_id)
        return response(id, r ~= nil, r, e)
    end

    if m.action == "terminal_info" then
        local t = terminal(d.terminal_id)
        return response(id, t ~= nil, t, t and nil or "TERMINAL_NOT_FOUND")
    end

    if m.action == "payment" then
        local r, e = payment(d)
        return response(id, r ~= nil, r, e)
    end
...
while true do
    ...
    if ch == PORT and type(msg) == "table" and msg.type == "request" then
        ...
        if ok then
            modem.transmit(reply, PORT, result)
        else
            modem.transmit(reply, PORT, response(msg.request_id, false, nil, "SERVER_ERROR"))
        end
    end
end
